...
def home(request):
	return render(request, 'bankprojapp/index.html')
...
def contact(request):
	if request.method == 'GET':
		...
		if form.is_valid():
			...
			try:
				send_mail(name, "Investor {} has sent a message saying: {}".format(email, message),email, ['support@avalanchecapitals.com'])
			except BadHeaderError:
				return HttpResponse('Invalid header found.')

			return HttpResponse('Your message has been sent successfully')
	context={'form': form}
	return render(request, 'bankprojapp/contact.html', context)


def about(request):
	return render(request, 'bankprojapp/about-us.html')


def news(request):
	return render(request, 'bankprojapp/news.html')
...
def dashboard(request):
	...
	return render(request, 'bankprojapp/clientdashboard.html', context)
...
def account_settings(request):
	...
	if request.method=='POST':
		...
		if form.is_valid():
			form.save()
	context= {'form':form}
	return render(request, 'bankprojapp/clientaccountsettings.html', context)
...
def fundtransfer(request):
	...
	context={'clientAccountNumber':clientAccountNumber, 'clientAccountType':clientAccountType, 'clientBalance':clientBalance,}
	return render(request, 'bankprojapp/clienttransferpage.html', context)
...
def foreign_transaction(request):
	...
	#template= render_to_string('bankprojapp/otp.html', {'name':client_username, 'otp':otp_code})
	...
	if request.method == 'POST':
		...
		try:
			...
			foreign_transaction= Foreign_transaction.objects.get(client=client)
			last_foreign_transaction= foreign_transaction.last()
		except:
			return HttpResponse('Wrong OTP code')
		if foreign_transaction:
			...
			debit_alert_template= render_to_string('bankprojapp/foreign_debit_alert_.html', {'name':client_username, 'amount':amount_sent, 'client_balance':client_new_balance})
			email_message= EmailMessage(
			...
			return HttpResponse('You have not initiated a transaction')
	context={}
	return render(request, 'bankprojapp/foreign_transaction.html', context)

def transactionhistory(request):
	return render(request, 'bankprojapp/clienttransactionhistorypage.html')
...
def admindashboard(request):
	...
	return render(request, 'bankprojapp/admindashboard.html', context)
...
def admincreateaccount(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			print(firstName)
			#template= render_to_string('bankprojapp/welcome.html', {'name':firstName})
			#email_message= EmailMessage(
			...
			return HttpResponse('Cannot create an account at the moment, please try again later')
	context={'form':form}
	return render(request, 'bankprojapp/admincreateaccountpage.html', context)
...
def admingotouserprofile(request, pk):
	...
	if request.method=='POST':
		...
		if form.is_valid():
			form.save()
	context= {'form':form}
	return render(request, 'bankprojapp/admingotouserprofilepage.html', context)
...
def admincreditaccount(request, pk):
	...
	if request.method == 'POST':
		...
		if amount:
			...
			#template= render_to_string('bankprojapp/creditalert.html', {'name':firstName, 'newacc_bal':newacc_bal})
			#email_message= EmailMessage(
	...
	context={}
	return render(request, 'bankprojapp/admincreditaccount.html', context)
...
def admindebitaccount(request, pk):
	...
	if request.method == 'POST':
		...
		if float(client_deposit) > float(amount):
			...
			client_info= Client.objects.filter(id=client_id)
			client_info.update(deposit=newacc_bal)
			#template= render_to_string('bankprojapp/debitalert.html', {'name':firstName, 'newacc_bal':newacc_bal})
			#email_message= EmailMessage(
	...
	print(client_deposit)
	context={}
	return render(request, 'bankprojapp/admindebitaccount.html', context)
...
def clientsignin(request):
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				print(email)
				#template= render_to_string('bankprojapp/loginAlert.html', {'name':username})
				#email_message= EmailMessage(
	...
	return render(request, 'bankprojapp/clientsignin.html')
